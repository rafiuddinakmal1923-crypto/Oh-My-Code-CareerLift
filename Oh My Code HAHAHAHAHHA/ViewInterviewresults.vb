...
Public Class ViewInterviewresults
    Dim SelectedEmail As String = ""

    Private Sub ViewInterviewresults_Load(sender As Object, e As EventArgs) Handles MyBase.Activated
    ...
    Private Sub LoadInterviewApplications()
        ...
        Dim query As String = "SELECT * FROM [JobSeeker Application] WHERE [Email] = ? AND ([Application Status] = 'Welcome On Board' OR [Application Status] = 'Interview')"
        ...
        Try
            ...
            If Not reader.HasRows Then
                reader.Close()
                MessageBox.Show("You have no interview statuses at the moment. Redirecting to job listings.", "No Interviews", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Form14.Show()
                Me.Hide()
                Exit Sub
            End If
    ...
    Private Sub LoadInterviewDetails(email As String, company As String)
        ...
        Try
            ...
            If reader.Read() Then
                ...
                End If
                If Not IsDBNull(reader("Interview Mode")) Then
                    txtMode.Text = reader("Interview Mode").ToString()
                End If
                If Not IsDBNull(reader("Application Status")) Then
                    Dim actualStatus As String = reader("Application Status").ToString()
                    If actualStatus = "Interview" Then
                        lblStatus.Text = "Pending Interview Result"
                    Else
                        lblStatus.Text = actualStatus
                    End If
                End If
            End If
            reader.Close()
    ...
    Private Sub btnInviteForInterview_Click(sender As Object, e As EventArgs) Handles btnInviteForInterview.Click
        ShowOfferLetter(SelectedEmail, lblCompanyFullName.Text)
    End Sub

    Private Sub ShowOfferLetter(email As String, company As String)
        If con.State = ConnectionState.Open Then con.Close()

        Try
            con.Open()
            Dim query As String = "SELECT [Offer Letter] FROM [JobSeeker Application] WHERE [Email]=? AND [Company Applied]=?"
            Dim cmd As New OleDbCommand(query, con)
            cmd.Parameters.AddWithValue("?", email)
            cmd.Parameters.AddWithValue("?", company)
            ...
            If result IsNot Nothing AndAlso Not IsDBNull(result) Then
                ...
                Dim pdfData() As Byte = CType(result, Byte())

                ' Simpan ke fail sementara
                Dim tempPath As String = Path.Combine(Path.GetTempPath(), "OfferLetter_" & Guid.NewGuid().ToString() & ".pdf")
                File.WriteAllBytes(tempPath, pdfData)

                ' Buka PDF
                Process.Start(tempPath)
            Else
                MessageBox.Show("No Offer Letter uploaded yet.", "Missing", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If

        Catch ex As Exception
            MessageBox.Show("Error viewing Offer Letter: " & ex.Message)
        Finally
            con.Close()
        End Try
    End Sub
